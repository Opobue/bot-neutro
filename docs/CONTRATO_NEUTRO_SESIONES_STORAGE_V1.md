...
- `transcript` (string, opcional): **sensible**. **DEFAULT: NO se persiste**. Solo puede persistirse en L2 con flag explícito (p. ej. `AUDIO_SESSION_PERSIST_TRANSCRIPT=1`) y **MUST** cumplir TTL máximo sensible (ver Retención).
- `reply_text` (string, opcional): **sensible**. **DEFAULT: NO se persiste**. Solo puede persistirse en L2 con flag explícito (p. ej. `AUDIO_SESSION_PERSIST_REPLY_TEXT=1`) y **MUST** cumplir TTL máximo sensible (ver Retención).
...
- Retención configurable por ENV (a implementar en L2):
  - `AUDIO_SESSION_RETENTION_DAYS` (entero, default 30, **MUST** estar en rango [0..30]).
...
- Si `transcript` y/o `reply_text` se persisten (flags habilitados), `expires_at` **MUST** calcularse como `min(created_at + 1 día, created_at + AUDIO_SESSION_RETENTION_DAYS)`.
...
- **Purga deshabilitada:** si `AUDIO_SESSION_PURGE_ENABLED=0`, no hay purga automática, pero `expires_at` sigue siendo obligatorio, **no** se habilitan lecturas/listados, y entornos productivos **MUST** aplicar un cap de almacenamiento o habilitar purga para evitar crecimiento infinito.
